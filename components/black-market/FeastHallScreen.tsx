...
function formatCost(cost: Partial<Record<string, number>>) {
  ...
  return costEntries
    .map(([resource, amount]) => `${amount} ${resource === "bioSamples" ? "Bio Samples" : resource.charAt(0).toUpperCase() + resource.slice(1)}`)
    .join(" · ");
}
...
export default function FeastHallScreen({
  ...
  function canAffordOffer(cost: Partial<Record<string, number>>) {
    const costEntries = Object.entries(cost).filter(
      (entry): entry is [string, number] => typeof entry[1] === "number",
    );

    return costEntries.every(([resource, amount]) => {
      const available = player.resources[resource as keyof typeof player.resources] ?? 0;
      return available >= amount;
    });
  }

  function handleUseOffer(offerId: (typeof feastHallOffers)[number]["id"]) {
  ...
  const content = (
    <>
        <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <SectionCard
            title="Lane Brief"
            description="Book 1 scope: one practical Black Market service line focused on survival pressure, recovery, and expedition readiness."
          >
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-amber-400/20 bg-amber-500/10 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-white/72">
                  Deals are sacred in the Black Market. The Feast Hall honors that law with fixed terms, clean payment, and no haggling after the first bite.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-white/72">
                  The lane draws from The Mouth of Inti and Pure ritual excess, but sells survival first: eat now, endure longer, pay in credits or salvage.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-white/72">
                  Prepare in the citadel, restore what the front took, then return to forging and expeditions without a new economy or extra subsystem.
                </p>
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard
            ...
            description="Shared state readout pulled from the existing condition, cooldown, and resource systems."
          >
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                ...
                <div className="mt-2 text-sm text-white/65">
                  {isRecoveryOnCooldown
                    ...
                    : "No recovery lockout is active. Any Feast Hall contract can be taken now if you can pay."}
                </div>
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3">
                    <div className="text-[10px] uppercase tracking-[0.18em] text-white/45">Credits</div>
                    <div className="mt-1 font-semibold text-white">{player.resources.credits}</div>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3">
                    <div className="text-[10px] uppercase tracking-[0.18em] text-white/45">Bio Samples</div>
                  ...
                  <div className="grid gap-3 text-sm text-white/75">
                    <div className="flex items-start gap-3 rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-white/65">{formatCost(offer.cost)}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-white/65">Restore {offer.conditionGain} condition and set a {Math.ceil(offer.cooldownMs / 1000)}s hall cooldown.</div>
                      </div>
                    </div>
